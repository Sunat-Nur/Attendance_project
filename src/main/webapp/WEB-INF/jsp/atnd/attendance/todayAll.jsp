...
<div class="searchBar">
    ...
    <div style="position: relative; display: inline-block; margin-right: 10px;">
        <input type="time" id="startTimePicker" class="timePickerInputFielstyle" padding-right: 30px; font-size: 12px;">
    </div>
    <div style="position: relative; display: inline-block;">
        <input type="time" id="endTimePicker"  class="timePickerInputFielstyle" padding-right: 30px; font-size: 12px;">
    </div>
</div>



<center><h4>TODAY ALL ATTENDANCE LIST</h4></center>
...
<thead>
	...
	<c:set var="loggedInUserSecurityLevel" value="${loggedInUser.securityLevel}" />
	<tbody>
	    <c:forEach var="attendance" items="${todayAllAttendanceList}">
	        <!-- Fetching user details for the current attendance record -->
	        <c:set var="userDetails" value="${userDetailsList.stream()
	                                          .filter(user -> user.empId == attendance.empId && user.securityLevel >= loggedInUserSecurityLevel)
	                                          .findFirst().orElse(null)}" />
	        <!-- Display the user details if they meet the security level criteria -->
